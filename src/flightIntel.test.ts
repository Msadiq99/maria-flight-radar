...
describe('flight intelligence', () => {
  ...
  it('predicts a near-zero closest approach for a direct track', () => {
    const prediction = predictFlight({ ...aircraft, updated_at: Date.now() }, 0, 1, 150, 1000)
  ...
  it('accounts for altitude and vertical motion in 3D closest approach', () => {
    ...
    expect(prediction.closest_3d_distance_km).toBeGreaterThanOrEqual(prediction.closest_distance_km)
