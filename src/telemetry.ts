...
async function fetchLatest(deviceId: string, signal: AbortSignal): Promise<TelemetryRecord | null> {
  const res = await fetch(`${API_BASE_URL}/api/telemetry/latest?device_id=${deviceId}`, { signal })
  if (res.status === 404) {
    return null
  }
  if (!res.ok) {
...
async function fetchHistory(deviceId: string, signal: AbortSignal): Promise<TelemetryRecord[]> {
  const res = await fetch(`${API_BASE_URL}/api/telemetry/history?device_id=${deviceId}&limit=500`, { signal })
  if (!res.ok) {
    throw new Error(`backend returned ${res.status}`)
  }
  return res.json()
...
export function useTelemetry(deviceId = DEVICE_ID) {
  ...
  useEffect(() => {
    ...
    stream.addEventListener('telemetry', (event) => {
      const incoming = JSON.parse((event as MessageEvent).data) as TelemetryRecord
      setRecord(incoming)
      setUnreachable(false)
      setHistory((current) => {
