...
const STATUS_COPY: Record<DeviceStatus, { label: string; className: string }> =
  {
    waiting: {
      label: 'Waiting for first telemetry',
    ...
    <div className="stat-card">
      ...
      {secondsSinceUpdate !== null && status !== 'unreachable' && (
        <span className="status-detail">
          Last seen {secondsSinceUpdate}s ago
        </span>
      )}
    </div>
  );
...
function AircraftRow({ aircraft }: { aircraft: FlightPrediction }) {
  const climb =
    aircraft.vertical_rate_mps > 0.5
      ? 'Climb'
      : aircraft.vertical_rate_mps < -0.5
        ? 'Descend'
        : 'Level';
...
function DashboardScreen() {
  ...
  const [selectedDeviceId, setSelectedDeviceId] = useState(
    sharedParams.get('device') || DEVICE_ID
  );
  const { record, history, status, secondsSinceUpdate } =
    useTelemetry(selectedDeviceId);
  ...
  const [radiusKm, setRadiusKm] = useState(
    Number(sharedParams.get('radius')) || 50
  );
  const [sortKey, setSortKey] = useState<AircraftSortKey>('flyby_probability');
  const [sortDirection, setSortDirection] = useState<SortDirection>('desc');
  const [playbackIndex, setPlaybackIndex] = useState<number | null>(null);
  const [savedLocations, setSavedLocations] = usePersistentState<
    SavedLocation[]
  >('maria.saved-locations', []);
  const [alertRules, setAlertRules] = usePersistentState<AlertRules>(
    'maria.alert-rules',
    DEFAULT_ALERT_RULES
  );
  ...
  const targetLat = monitorLat.trim()
    ? Number(monitorLat)
    : hasFix && record
      ? record.gps.lat
      : null;
  const targetLon = monitorLon.trim()
    ? Number(monitorLon)
    : hasFix && record
      ? record.gps.lon
      : null;
  ...
  } = useNearbyTraffic(validTargetLat, validTargetLon, radiusKm);
  const fixedHistory = history.filter((entry) => entry.gps.fix);
  const trail = fixedHistory.map((entry): [number, number] => [
    entry.gps.lat,
    entry.gps.lon,
  ]);
  const playbackRecord =
    playbackIndex === null ? record : (fixedHistory[playbackIndex] ?? record);
  const packetStats = useMemo(() => {
    ...
    const avgTemp =
      history.reduce((sum, entry) => sum + entry.imu.temp_c, 0) /
      history.length;
    return {
  ...
  const trafficStats = useMemo(() => {
    ...
    return {
      ...
      climbing: predictedAircraft.filter((item) => item.vertical_rate_mps > 0.5)
        .length,
    };
  }, [predictedAircraft]);
  ...
  useEffect(() => {
    ...
    const quiet =
      alertRules.quietStart > alertRules.quietEnd
    ...
    for (const item of predictedAircraft) {
      const matches =
        ...
        (!alertRules.callsign ||
          item.callsign
            .toLowerCase()
            .includes(alertRules.callsign.toLowerCase())) &&
        (!alertRules.airline ||
          item.airline
            .toLowerCase()
            .includes(alertRules.airline.toLowerCase()));
  ...
  async function copyShareLink() {
    ...
    if (validTargetLat !== null)
      url.searchParams.set('lat', String(validTargetLat));
    if (validTargetLon !== null)
      url.searchParams.set('lon', String(validTargetLon));
  ...
  return (
    <main className="app-shell">
      ...
      <header className="hero-panel">
        ...
        <div className="hero-stats" aria-label="Device status overview">
          ...
          <div className="stat-card">
            ...
            <strong>
              {record ? `${record.gps.speed_kmph.toFixed(1)} km/h` : '—'}
            </strong>
          </div>
        </div>
      </header>
      ...
      <section className="map-panel" id="map">
        {validTargetLat !== null && validTargetLon !== null ? (
          ...
          <div className="map-placeholder">
            <p>No GPS fix yet</p>
            <span>
              The tracker hasn't reported a valid position. The map appears once
              it does.
            </span>
          </div>
        )}
      ...
      <section className="panel" id="alerts">
        ...
        <div className="control-grid">
          ...
          <label>
            ...
            <input
              value={alertRules.callsign}
              onChange={(event) =>
                setAlertRules({ ...alertRules, callsign: event.target.value })
              }
            />
          </label>
          <label>
            ...
            <input
              value={alertRules.airline}
              onChange={(event) =>
                setAlertRules({ ...alertRules, airline: event.target.value })
              }
            />
          </label>
          <label>
      ...
      <section className="panel">
        ...
        <div className="stat-grid">
          <StatTile
            ...
            value={`${predictedAircraft.length}`}
          />
          <StatTile label="Traffic source" value={trafficSourceLabel} />
          <StatTile
            label="Highest"
            value={
              trafficStats.highest === null
                ? '—'
                : `${trafficStats.highest.toLocaleString()} m`
            }
          />
          <StatTile
            label="Fastest"
            value={
              trafficStats.fastest === null
                ? '—'
                : `${trafficStats.fastest.toFixed(0)} km/h`
            }
          />
        </div>
        <div className="control-grid">
          <label>
            ...
            <input
              value={monitorLat}
              onChange={(event) => setMonitorLat(event.target.value)}
              placeholder={record?.gps.lat.toFixed(6) ?? '24.713600'}
            />
          </label>
          <label>
          ...
          <label>
            ...
            <input
              value={monitorLon}
              onChange={(event) => setMonitorLon(event.target.value)}
              placeholder={record?.gps.lon.toFixed(6) ?? '46.675300'}
            />
          </label>
          <label>
            <span>Radius</span>
            <input
              ...
              value={radiusKm}
              onChange={(event) =>
                setRadiusKm(Number(event.target.value) || 50)
              }
            />
          </label>
        </div>
            ...
            <tbody>
              {sortedAircraft.length > 0 ? (
                sortedAircraft.map((item) => (
                  <AircraftRow key={item.id} aircraft={item} />
                ))
              ) : (
                <tr>
              ...
              {flybySchedule.length > 0 ? (
                flybySchedule.map((item) => (
                  <tr key={`${item.id}-flyby`}>
                    ...
                    <td>
                      <a href={item.fr24_url} target="_blank" rel="noreferrer">
                        FR24
                      </a>
                      {' · '}
                      <a
                        href={item.skybrary_url}
                        target="_blank"
                        rel="noreferrer"
                      >
                        Skybrary
                ...
                <tr>
                  <td colSpan={8}>
                    No predicted flybys inside the current radius
                  </td>
                </tr>
              )}
            </tbody>
      ...
      <section className="panel" id="history">
        ...
        <div className="stat-grid">
          <StatTile
            label="Packets with fix"
            value={`${packetStats.fixes}/${history.length}`}
          />
          <StatTile
            label="Max speed"
            value={
              packetStats.maxSpeed === null
                ? '—'
                : `${packetStats.maxSpeed.toFixed(1)} km/h`
            }
          />
          <StatTile
            label="Average IMU temp"
            value={
              packetStats.avgTemp === null
                ? '—'
                : `${packetStats.avgTemp.toFixed(1)}°C`
            }
          />
          ...
          <StatTile
            ...
            value={
              record?.diagnostics
                ? `${record.diagnostics.battery_percent}%`
                : '—'
            }
          />
          <StatTile
            ...
            value={
              record?.diagnostics
                ? `${record.diagnostics.wifi_rssi_dbm} dBm`
                : '—'
            }
          />
          <StatTile
            label="Firmware"
            value={record?.diagnostics?.firmware_version ?? '—'}
          />
          <StatTile
            label="Delivery failures"
            value={
              record?.diagnostics
                ? `${record.diagnostics.delivery_failures}`
                : '—'
            }
          />
        </div>
        {fixedHistory.length > 1 && (
          <div className="control-grid">
            ...
            <button type="button" onClick={() => setPlaybackIndex(null)}>
              Return to live
            </button>
            <span>
              {playbackIndex === null
                ? 'Live'
                : formatTime(
                    fixedHistory[playbackIndex]?.received_at ?? Date.now()
                  )}
            </span>
          </div>
        )}
        <div className="control-grid">
          <button
            type="button"
            onClick={() => exportTelemetryCsv(history, selectedDeviceId)}
          >
            {t('exportCsv')}
          </button>
          <button
            type="button"
            onClick={() => exportTelemetryGeoJson(history, selectedDeviceId)}
          >
            {t('exportGeoJson')}
          </button>
          ...
          <table>
            ...
            <tbody>
              {recentPackets.length > 0 ? (
                recentPackets.map((entry) => (
                  <PacketRow key={entry.received_at} record={entry} />
                ))
              ) : (
                <tr>
      ...
      <section className="panel">
        ...
        <div className="stat-grid">
          <StatTile
            label="Latitude"
            value={record ? record.gps.lat.toFixed(6) : '—'}
          />
          <StatTile
            label="Longitude"
            value={record ? record.gps.lon.toFixed(6) : '—'}
          />
          <StatTile
            label="Altitude"
            value={record ? `${record.gps.alt_m.toFixed(1)} m` : '—'}
          />
          <StatTile
            label="Course"
            value={record ? `${record.gps.course_deg.toFixed(1)}°` : '—'}
          />
        </div>
      </section>
      ...
      <section className="panel">
        <div className="panel-header">
          ...
          <p>
            {record ? (record.imu.valid ? 'Sensor OK' : 'No sensor data') : '—'}
          </p>
        </div>
        <div className="stat-grid">
          <StatTile
            label="Accel X"
            value={record ? `${record.imu.accel.x.toFixed(2)} g` : '—'}
          />
          <StatTile
            label="Accel Y"
            value={record ? `${record.imu.accel.y.toFixed(2)} g` : '—'}
          />
          <StatTile
            label="Accel Z"
            value={record ? `${record.imu.accel.z.toFixed(2)} g` : '—'}
          />
          <StatTile
            label="Gyro X"
            value={record ? `${record.imu.gyro.x.toFixed(2)} deg/s` : '—'}
          />
          <StatTile
            label="Gyro Y"
            value={record ? `${record.imu.gyro.y.toFixed(2)} deg/s` : '—'}
          />
          <StatTile
            label="Gyro Z"
            value={record ? `${record.imu.gyro.z.toFixed(2)} deg/s` : '—'}
          />
          <StatTile
            label="Temperature"
            value={record ? `${record.imu.temp_c.toFixed(1)}°C` : '—'}
          />
        </div>
      </section>
    </main>
