...
export function exportTelemetryGeoJson(
  records: TelemetryRecord[],
  deviceId: string
) {
  const geojson = {
    ...
    features: records
      .filter((record) => record.gps.fix)
      .map((record) => ({
        type: 'Feature',
        geometry: {
          type: 'Point',
          coordinates: [record.gps.lon, record.gps.lat, record.gps.alt_m],
        },
        properties: {
