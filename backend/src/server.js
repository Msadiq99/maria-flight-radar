...
const DB_PATH = process.env.DB_PATH || resolve(MODULE_DIR, '../data/maria.sqlite')
...
const AIRPORTS = [
  { code: 'RUH', name: 'King Khalid International', lat: 24.9576, lon: 46.6988, runways: ['15L/33R', '15R/33L'] },
  { code: 'JED', name: 'King Abdulaziz International', lat: 21.6796, lon: 39.1565, runways: ['16L/34R', '16C/34C', '16R/34L'] },
  { code: 'DMM', name: 'King Fahd International', lat: 26.4712, lon: 49.7979, runways: ['16L/34R', '16R/34L'] },
  { code: 'MED', name: 'Prince Mohammad bin Abdulaziz', lat: 24.5534, lon: 39.7051, runways: ['17/35', '18/36'] },
]
...
app.use((req, res, next) => {
  ...
  req.requestId = suppliedId && /^[A-Za-z0-9._-]{1,128}$/.test(suppliedId)
    ? suppliedId
    : randomUUID()
...
app.use(cors({
  origin(origin, callback) {
    if (!origin || (!IS_PRODUCTION && CORS_ORIGINS.length === 0) || CORS_ORIGINS.includes(origin)) {
...
function isValidTelemetry(body) {
  ...
  return (
    ...
    (body.sequence === undefined || (
      Number.isInteger(body.sequence) && inRange(body.sequence, 0, 0xffffffff)
    )) &&
    (body.captured_at === undefined || (
      Number.isInteger(body.captured_at) && inRange(body.captured_at, 0, Number.MAX_SAFE_INTEGER)
    )) &&
...
app.get('/api/telemetry/latest', (req, res) => {
  ...
  if (!validDeviceId(deviceId)) {
    return res.status(400).json({ error: 'valid device_id query param required' })
  }
...
app.get('/api/telemetry/playback', (req, res) => {
  ...
  if (!validDeviceId(deviceId) || from < 0 || to < from) {
    return res.status(400).json({ error: 'valid device_id, from, and to are required' })
  }
...
app.get('/metrics', (_req, res) => {
  const averageResponseMs = metrics.requests > 0
    ? metrics.responseTimeMs / metrics.requests
    : 0
  res.json({
    ...metrics,
