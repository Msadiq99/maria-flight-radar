...
test('verifies telemetry HMAC signatures', () => {
  ...
  assert.equal(isValidTelemetrySignature(body, '0'.repeat(64), 'test-secret'), false)
})
...
test('ingests telemetry and returns the latest record', async () => {
  ...
  const latest = await fetch(`${baseUrl}/api/telemetry/latest?device_id=MARIA-001`)
  ...
  const history = await fetch(`${baseUrl}/api/telemetry/history?device_id=MARIA-001&limit=10`)
  assert.equal((await history.json()).length, 1)
})

test('rejects invalid traffic coordinates', async () => {
  const response = await fetch(`${baseUrl}/api/traffic/nearby?lat=91&lon=46&radius_km=50`)
